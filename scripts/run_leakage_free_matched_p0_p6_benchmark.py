#!/usr/bin/env python3
"""
Canonical Leakage-Free Matched P0-P6 Benchmark Runner
=====================================================
Digital Finance Forensic Governance Overhaul
Date: September 2026

Executes the complete, leakage-free matched comparison across 8 systems:
- P0:  Primary Learned Memory (Global, Cross-Market, 0% same-ticker)
- P0*: Domestic Guardrail Memory (Within-Market, 0% same-ticker)
- P1:  Parametric Backbone (Ablated Memory, No Retrieval)
- P2:  Linear Ridge / Mean-Only Memory
- P3:  Clean Raw-Feature kNN (Strict 0% same-ticker entity leakage)
- P4:  Momentum-21 Ranking Baseline
- P5:  Random Ranking Null
- P6:  Equal-Weight Buy & Hold Context

Frozen Contract Specifications:
1. 252 x 23 historical observation window (42 weekly patches, P=6 sessions).
2. Frozen Patch-Transformer checkpoints for seeds 7, 17, 37.
3. Causal memory bank strictly <= 2020-12-31 with mature 63-session forward outcomes.
4. Strict causal execution timing: Signal at session t Close, Fill at session t+1 Open (10 bps fee).
5. Dynamic 2.5 x ATR_14 Chandelier trailing stop (10% floor) with max holding 63 sessions.
6. 3 concurrent slots per sovereign market.
7. Strict 0.0% entity leakage (ticker self-exclusion query_ticker != neighbor_ticker) across ALL systems.
8. Panel-preserving moving-block bootstrap (B=10,000, L=21) testing Delta Sharpe with Benjamini-Hochberg FDR.
"""
...
OUTPUT_DIR = PROJECT_ROOT / "paper" / "internal" / "evidence" / "canonical_benchmark_reproduction"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
t_bench_start = time.time()
print("=" * 85)
print(f"CANONICAL LEAKAGE-FREE MATCHED P0-P6 BENCHMARK RUNNER")
...
print("1. Ingesting raw market data and computing 23 causal technical features...", flush=True)
market_data = {}
all_tickers_global = []
ticker_to_id = {}
...
print("2. Loading authoritative frozen Patch-Transformer checkpoints (Seeds 7, 17, 37)...", flush=True)
...
for s in SEEDS:
    ...
    checkpoint_hashes[s] = ckpt_hash
    model = AnnualPatchTemporalTransformer(input_dim=23, embed_dim=64, num_heads=4, latent_dim=128).to(DEVICE)
    state = torch.load(ckpt_path, map_location=DEVICE, weights_only=True)
    model.load_state_dict(state)
    model.eval()
    encoders[s] = model
    print(f"   [+] Seed {s:2d} Loaded | SHA-256: {ckpt_hash[:16]}...")

print("3. Indexing Causal Historical Memory Bank (Mature Outcomes <= 2020-12-31)...", flush=True)
mem_records_base = []
mem_patch_list = []
...
print(f"   [+] Indexed {len(mem_records_base)} causal annual episodes <= 2020-12-31.")
...
SYSTEM_CONFIGS = {
    ...
    "P3": {"name": "Raw-feature kNN memory (Clean)", "claim": "H1/H2 Control (Clean)"},
...
print("4. Simulating Matched 144-Cell Execution Grid (8 Systems x 6 Markets x 3 Seeds)...", flush=True)
matrix_rows = []
all_equity_curves = []
all_trade_ledgers = []
all_decision_ledgers = []
all_25_neighbor_rows = []
                        ...
                        if open_p > 0:
                            ...
                            if shares > 0:
                                ...
                                if cash >= (cost + fee):
                                    ...
                                    # Log 25 neighbors for retrieval systems
                                    if "nbr_details" in ent:
                                        for n_rank, n_item in enumerate(ent["nbr_details"], 1):
                                            all_25_neighbor_rows.append({
                                                "trade_id": trade_id,
                                                "market": m,
                                                "seed": s,
                                                "system": sys_id,
                                                "query_ticker": tickers[t_i],
                                                "decision_date": ent["signal_date"],
                                                "execution_date": cur_date_str,
                                                "neighbor_rank": n_rank,
                                                "neighbor_market": n_item["market"],
                                                "neighbor_ticker": n_item["ticker"],
                                                "neighbor_date": n_item["date"],
                                                "cosine_similarity": round(float(n_item["sim"]), 4),
                                                "normalized_weight": round(float(n_item["weight"]), 4),
                                                "realized_return_63d": round(float(n_item["ret63"]), 4),
                                                "realized_drawdown_63d": round(float(n_item["dd63"]), 4),
                                            })
                    pending_entries = []
                            ...
                            if available_indices:
                                ...
                                for idx_c, t_i in enumerate(available_indices):
                                    ...
                                    if sys_id in ("P0", "P0*", "P2"):
                                        ...
                                        # Strict 0% same-ticker leakage guardrail
                                        if sys_id == "P0*":
                                        ...
                                        # Sort 25 neighbors descending by weight
                                        sort_order = np.argsort(-weights)
                                        sorted_nbrs = valid_nbrs[sort_order]
                                        sorted_w = weights[sort_order]
                                        sorted_sims = nbr_sims[sort_order]
                                        sorted_rets = nbr_ret[sort_order]
                                        sorted_dds = nbr_dd[sort_order]
                                        
                                        nbr_details = [
                                            {
                                                "market": mem_market_arr[sorted_nbrs[n]],
                                                "ticker": mem_tickers_arr[sorted_nbrs[n]],
                                                "date": mem_dates_arr[sorted_nbrs[n]],
                                                "sim": sorted_sims[n],
                                                "weight": sorted_w[n],
                                                "ret63": sorted_rets[n],
                                                "dd63": sorted_dds[n],
                                        ...
                                        # STRICT CLEAN P3: 0.0% same-ticker entity leakage
                                        q_raw = raw_feats_2024[day_idx, t_i]
                                        dist = np.linalg.norm(mem_raw_feats - q_raw, axis=1)
                                        valid_mask = (mem_tickers_arr != q_tkr) # STRICT EXCLUSION
                                        valid_indices = np.where(valid_mask)[0]
                                        ...
                                        nbr_details = [
                                            {
                                                "market": mem_market_arr[valid_nbrs[n]],
                                                "ticker": mem_tickers_arr[valid_nbrs[n]],
                                                "date": mem_dates_arr[valid_nbrs[n]],
                                                "sim": 1.0 / (1.0 + dist[valid_nbrs[n]]),
                                                "weight": 0.04,
                                                "ret63": mem_ret63[valid_nbrs[n]],
                                                "dd63": mem_dd63[valid_nbrs[n]],
                                            }
                                            for n in range(25)
                                        ]
                                        decision_details.append({
...
df_neighbors = pd.DataFrame(all_25_neighbor_rows)

print("5. Computing Panel-Preserving Moving-Block Bootstrap for Delta Sharpe (B=10,000, L=21)...", flush=True)
...
comparators = [
    ...
    ("P0", "P3", "P0 vs P3 (H1/H2 Control: Clean kNN)"),
...
print("6. Exporting Canonical Empirical Artifacts...", flush=True)
df_matrix.to_csv(OUTPUT_DIR / "canonical_144_cell_performance_matrix.csv", index=False)
df_curves.to_csv(OUTPUT_DIR / "canonical_daily_equity_curves.csv", index=False)
df_trades.to_csv(OUTPUT_DIR / "canonical_trade_execution_ledger.csv", index=False)
df_decisions.to_csv(OUTPUT_DIR / "canonical_query_neighbor_decision_ledger.csv", index=False)
df_bootstrap.to_csv(OUTPUT_DIR / "canonical_paired_panel_bootstrap.csv", index=False)
df_neighbors.to_csv(OUTPUT_DIR / "canonical_25_neighbor_ledger.csv", index=False)

# Export copy to canonical_benchmark_outputs
CANON_PUBLIC = PROJECT_ROOT / "canonical_benchmark_outputs"
CANON_PUBLIC.mkdir(parents=True, exist_ok=True)
df_matrix.to_csv(CANON_PUBLIC / "canonical_144_cell_performance_matrix.csv", index=False)
df_curves.to_csv(CANON_PUBLIC / "canonical_daily_equity_curves.csv", index=False)
df_trades.to_csv(CANON_PUBLIC / "canonical_trade_execution_ledger.csv", index=False)
df_decisions.to_csv(CANON_PUBLIC / "canonical_query_neighbor_decision_ledger.csv", index=False)
df_bootstrap.to_csv(CANON_PUBLIC / "canonical_paired_panel_bootstrap.csv", index=False)
df_neighbors.to_csv(CANON_PUBLIC / "canonical_25_neighbor_ledger.csv", index=False)

# Metadata
meta = {
    "execution_time_seconds": round(time.time() - t_bench_start, 1),
    "python_version": sys.version,
    "torch_version": torch.__version__,
    "cuda_device": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "CPU",
    "checkpoint_hashes": checkpoint_hashes,
    "data_manifest_sha256": "bdf79b3866b3659b1e14e41453f31d558bbc057ab3e7af7c706a97762c33d7f5",
    "systems_evaluated": list(SYSTEM_CONFIGS.keys()),
    "total_cells": len(df_matrix),
    "total_trades": len(df_trades),
    "total_decisions": len(df_decisions),
    "total_precedents": len(df_neighbors),
    "entity_leakage_p3_rows": int((df_neighbors[df_neighbors['system'] == 'P3']['query_ticker'] == df_neighbors[df_neighbors['system'] == 'P3']['neighbor_ticker']).sum()),
}
with open(OUTPUT_DIR / "canonical_run_metadata.json", "w") as f:
    json.dump(meta, f, indent=2)
with open(CANON_PUBLIC / "canonical_run_metadata.json", "w") as f:
    json.dump(meta, f, indent=2)

print("\n" + "="*85)
print("AUDITABLE 144-CELL PERFORMANCE SCORECARD (CLEAN COMPARISON)")
print("="*85)
summary_table = []
for sys_id, sys_cfg in SYSTEM_CONFIGS.items():
    ...
    summary_table.append({
        ...
        "Win Rate": f"{sub_m['win_rate'].mean()*100:.1f}%",
        "Med Hold": f"{sub_m['cell_median_hold'].mean():.1f}d",
        "Trade Count": len(sub_t),
    })
print(pd.DataFrame(summary_table).to_string(index=False))

print("\n" + "="*85)
print("PANEL BOOTSTRAP INFERENCE (DELTA SHARPE, B=10,000, L=21)")
print("="*85)
print(df_bootstrap.to_string(index=False))
print(f"\n[+] Execution completed in {time.time()-t_bench_start:.1f}s.")
