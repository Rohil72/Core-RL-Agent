"""
Batch 1: Small, Isolated Policy Experiments.

Implements and evaluates the 4 isolated policy arms against the Reconciled Reference (P0):
  - Experiment 1: Target-Scale Harmonization (P0_Harmonized)
  - Experiment 2: Loss-Only Downside Penalty (P0_LossOnly vs P0_ZeroPenalty vs P0_Legacy)
  - Experiment 3: Numerator Ranking vs Volatility Position Sizing (Arm A vs Arm B vs Arm C)
  - Experiment 5: Mean-Score Ensemble (P0_Ensemble)

Strict Verification Standards:
  - Phase 0 Reference Invariant Conformance (Zero entity leakage, zero lookahead)
  - Penny-level accounting identity: NAV_T - Initial_Capital = sum(Realized_Trade_PnL) + sum(Calendar_End_Exit_Fees)
...
RECONCILED_DIR = PROJECT_ROOT / "research_runs" / "reference_reconciled"
OUTPUT_DIR = PROJECT_ROOT / "research_runs" / "batch1_policy_experiments"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def run_batch1_experiments():
    print("=" * 85)
    print("BATCH 1: SMALL, ISOLATED POLICY EXPERIMENTS (EXPS 1, 2, 3, 5)")
    ...
    # 2. Ingest Data using exact Phase 0 pipeline
    ...
    # 4. Policy Arms to Execute
    SINGLE_SEED_ARMS = [
        "P0",
        "P0_Exp1_Harmonized",
        "P0_Exp2_LossOnly",
        "P0_Exp2_ZeroPenalty",
        "P0_Exp3_NumRank_EqualSize",
        "P0_Exp3_NumRank_InvVolSize",
    ]
    ...
    print("\n[Step 3] Executing Simulation Matrix & Verification Engines across All Sovereign Markets...")
    ...
    for m_idx, m in enumerate(TICKERS_BY_MARKET.keys()):
        ...
        real_ret63_2024 = np.zeros((n_sessions, len(tickers)), dtype=np.float32)
        ...
        for t_i, tkr in enumerate(tickers):
            ...
            for d_i, dt in enumerate(cal_dates):
                if dt in df.index:
                    ...
                    if "future_return_63" in df.columns and not np.isnan(df["future_return_63"].iloc[loc]):
                        real_ret63_2024[d_i, t_i] = float(df["future_return_63"].iloc[loc])
        ...
        # -------------------------------------------------------------
        # Part A: Single-Seed Arms
        # -------------------------------------------------------------
        for s in SEEDS:
            mem_lat_gpu = mem_latents_by_seed_gpu[s]

            for arm_id in SINGLE_SEED_ARMS:
                    ...
                    if day_idx < n_sessions - 1:
                        ...
                        if slots_open > 0 and day_idx < n_sessions - 5:
                            ...
                            if available_indices:
                                ...
                                numerators = np.zeros(len(available_indices))
                                vol_cands = np.zeros(len(available_indices))
                                ...
                                for idx_c, t_i in enumerate(available_indices):
                                    ...
                                    # Policy arm logic
                                    if arm_id == "P0":
                                        num = pred_val + 0.8 * mu_w - 0.2 * abs(cvar_05)
                                        sc = num / (v + 1e-4)
                                    elif arm_id == "P0_Exp1_Harmonized":
                                        mu_tr = target_moments[m]["mean"]
                                        sig_tr = target_moments[m]["std"]
                                        r_hat = pred_val * sig_tr + mu_tr
                                        num = r_hat + 0.8 * mu_w - 0.2 * abs(cvar_05)
                                        sc = num / (v + 1e-4)
                                        if s == 7 and idx_c == 0:
                                            diagnostics_exp1.append({
                                                "market": m,
                                                "date": cur_date_str,
                                                "ticker": q_tkr,
                                                "y_hat_raw": float(pred_val),
                                                "r_hat_harmonized": float(r_hat),
                                                "mu_w": float(mu_w),
                                                "abs_cvar": float(abs(cvar_05)),
                                                "vol": float(v),
                                                "legacy_score": float((pred_val + 0.8 * mu_w - 0.2 * abs(cvar_05)) / (v + 1e-4)),
                                                "harmonized_score": float(sc),
                                            })
                                    elif arm_id == "P0_Exp2_LossOnly":
                                        loss_penalty = max(0.0, -cvar_05)
                                        num = pred_val + 0.8 * mu_w - 0.2 * loss_penalty
                                        sc = num / (v + 1e-4)
                                        if s == 7:
                                            diagnostics_exp2.append({
                                                "market": m,
                                                "date": cur_date_str,
                                                "ticker": q_tkr,
                                                "cvar_05": float(cvar_05),
                                                "legacy_penalty": float(abs(cvar_05)),
                                                "loss_only_penalty": float(loss_penalty),
                                                "cvar_positive": int(cvar_05 > 0),
                                            })
                                    elif arm_id == "P0_Exp2_ZeroPenalty":
                                        num = pred_val + 0.8 * mu_w
                                        sc = num / (v + 1e-4)
                                    elif arm_id in ("P0_Exp3_NumRank_EqualSize", "P0_Exp3_NumRank_InvVolSize"):
                                        num = pred_val + 0.8 * mu_w - 0.2 * abs(cvar_05)
                                        sc = num
                                    else:
                                        num = pred_val + 0.8 * mu_w - 0.2 * abs(cvar_05)
                                    ...
                                    numerators[idx_c] = num
                                    ranking_scores[idx_c] = sc

                                # Rank candidates
                                top_order = np.argsort(-ranking_scores)
                                chosen_ranks = top_order[:slots_open]
                                j_count = len(chosen_ranks)

                                # Capital Allocation
                                if arm_id == "P0_Exp3_NumRank_InvVolSize":
                                    total_budget = (j_count / 3.0) * tot_equity
                                    chosen_vols = vol_cands[chosen_ranks]
                                    inv_vols = 1.0 / (chosen_vols + 1e-4)
                                    cap_weights = inv_vols / np.sum(inv_vols)
                                    alloc_caps = total_budget * cap_weights
                                    if s == 7:
                                        diagnostics_exp3.append({
                                            "market": m,
                                            "date": cur_date_str,
                                            "j_count": j_count,
                                            "total_budget": float(total_budget),
                                            "chosen_tickers": [tickers[available_indices[c]] for c in chosen_ranks],
                                            "chosen_vols": [float(v) for v in chosen_vols],
                                            "cap_weights": [float(w) for w in cap_weights],
                                        })
                                else:
                                    cap_per_slot = tot_equity / 3.0
                                    alloc_caps = [cap_per_slot] * j_count

                                for rank_pos, chosen_c in enumerate(chosen_ranks):
                                    real_ti = available_indices[chosen_c]
                                    pending_entries.append({
                                        "ticker_idx": real_ti,
                                        "signal_date": cur_date_str,
                                        "allocated_capital": alloc_caps[rank_pos],
                                    })
        ...
        # -------------------------------------------------------------
        # Part B: Exp 5 — Multi-Seed Mean-Score Ensemble
        # -------------------------------------------------------------
        arm_id = "P0_Exp5_Ensemble"
        initial_capital = 100000.0
        cash = initial_capital
        portfolio_equity = np.zeros(n_sessions)
        daily_returns = np.zeros(n_sessions)
        ...
        for day_idx in range(n_sessions):
                ...
                if open_p > 0:
                    ...
                    if shares > 0:
                        ...
                        if cash >= (cost + fee):
                            cash -= (cost + fee)
                            entry_seq += 1
                            trade_id = f"TRD_{m}_ENS_{entry_seq:03d}"
                            open_positions.append({
                                "trade_id": trade_id,
                                "ticker": tickers[t_i],
                                "ticker_idx": t_i,
            ...
            if day_idx < n_sessions - 1:
                ...
                if slots_open > 0 and day_idx < n_sessions - 5:
                    ...
                    if available_indices:
                        candidate_seed_scores = np.zeros((len(SEEDS), len(available_indices)))
                        ...
                        for s_i, s in enumerate(SEEDS):
                            ...
                            for idx_c, t_i in enumerate(available_indices):
                                ...
                                candidate_seed_scores[s_i, idx_c] = (pred_val + 0.8 * mu_w - 0.2 * abs(cvar_05)) / (v + 1e-4)

                        mean_scores = np.mean(candidate_seed_scores, axis=0)

                        cand_preds = np.array([session_preds[s][day_idx, available_indices] for s in SEEDS])
                        pred_std = np.std(cand_preds, axis=0)
                        for idx_c, t_i in enumerate(available_indices):
                            diagnostics_exp5.append({
                                "market": m,
                                "date": cur_date_str,
                                "ticker": tickers[t_i],
                                "pred_mean": float(np.mean(cand_preds[:, idx_c])),
                                "pred_disagreement_std": float(pred_std[idx_c]),
                                "realized_ret63": float(real_ret63_2024[day_idx, t_i]),
                                "abs_pred_error": float(abs(np.mean(cand_preds[:, idx_c]) - real_ret63_2024[day_idx, t_i])),
                            })

                        top_order = np.argsort(-mean_scores)
                        cap_per_slot = tot_equity / 3.0
    ...
    # 7. Statistical Inference: Calendar-Week Block Bootstrap
    ...
    for arm in arms_to_compare:
        ...
        if arm == "P0_Exp5_Ensemble":
            for m in TICKERS_BY_MARKET.keys():
                p0_m = [r for r in p0_sub if r["market"] == m]
                ens_m = [r for r in arm_sub if r["market"] == m][0]
                mean_p0_ret = np.mean([r["annualized_return"] for r in p0_m])
                mean_p0_sh = np.mean([r["sharpe"] for r in p0_m])
                diff_returns.append(ens_m["annualized_return"] - mean_p0_ret)
                diff_sharpes.append(ens_m["sharpe"] - mean_p0_sh)
        else:
    ...
    df_diag_e1 = pd.DataFrame(diagnostics_exp1)
    df_diag_e1.to_csv(OUTPUT_DIR / "mechanism_diagnostics_exp1.csv", index=False)

    df_diag_e2 = pd.DataFrame(diagnostics_exp2)
    cvar_pos_frac = float(df_diag_e2["cvar_positive"].mean()) if len(df_diag_e2) > 0 else 0.0
    df_diag_e2.to_csv(OUTPUT_DIR / "mechanism_diagnostics_exp2.csv", index=False)

    df_diag_e3 = pd.DataFrame(diagnostics_exp3)
    df_diag_e3.to_csv(OUTPUT_DIR / "mechanism_diagnostics_exp3.csv", index=False)

    df_diag_e5 = pd.DataFrame(diagnostics_exp5)
    disagree_corr = float(df_diag_e5["pred_disagreement_std"].corr(df_diag_e5["abs_pred_error"])) if len(df_diag_e5) > 0 else 0.0
    df_diag_e5.to_csv(OUTPUT_DIR / "mechanism_diagnostics_exp5.csv", index=False)
    ...
    audit_report = {
        ...
        "mechanism_findings": {
            "exp1_target_moments": target_moments,
            "exp2_cvar_positive_frequency": round(cvar_pos_frac, 4),
            "exp5_disagreement_error_correlation": round(disagree_corr, 4),
        },
    ...
    arm_names = {
        "P0": "Reconciled Baseline P0",
        "P0_Exp1_Harmonized": "Exp 1: Harmonized Target Scale",
        "P0_Exp2_LossOnly": "Exp 2: Loss-Only Tail Penalty",
        "P0_Exp2_ZeroPenalty": "Exp 2 Control: Zero Tail Penalty",
        "P0_Exp3_NumRank_EqualSize": "Exp 3 Arm B: Num-Rank + Equal Size",
        "P0_Exp3_NumRank_InvVolSize": "Exp 3 Arm C: Num-Rank + InvVol Size",
        "P0_Exp5_Ensemble": "Exp 5: Mean-Score Ensemble",
    }

    md = f"""# Batch 1: Small, Isolated Policy Experiments Summary
...
|---|---:|---:|---:|---:|---:|---:|
"""
    for arm_id in ["P0"] + arms_to_compare:
    ...
    md += """
## Paired Contrasts Against Reconciled P0 (Block Bootstrap Inference)
...
1. **Exp 1 (Score-Scale Harmonization):**
   - Pre-2021 training target moments: US ($\\mu={target_moments['US']['mean']:.4f}, \\sigma={target_moments['US']['std']:.4f}$), India ($\\mu={target_moments['India']['mean']:.4f}, \\sigma={target_moments['India']['std']:.4f}$), China ($\\mu={target_moments['China']['mean']:.4f}, \\sigma={target_moments['China']['std']:.4f}$), Brazil ($\\mu={target_moments['Brazil']['mean']:.4f}, \\sigma={target_moments['Brazil']['std']:.4f}$), France ($\\mu={target_moments['France']['mean']:.4f}, \\sigma={target_moments['France']['std']:.4f}$), UK ($\\mu={target_moments['UK']['mean']:.4f}, \\sigma={target_moments['UK']['std']:.4f}$).
   - Testing whether scaling $\\widehat{{r}} = \\widehat{{y}}\\sigma + \\mu$ realigns the trade-off with $0.8\\mu_w$.
2. **Exp 2 (Loss-Only Tail Penalty):**
   - Across all query snapshots in 2024, $\\text{{CVaR}}_{{0.05}} > 0$ occurred in **{cvar_pos_frac:.1%}** of candidate evaluations. In those regimes, the legacy $|\\text{{CVaR}}|$ penalized positive tails, whereas $\\max(0, -\\text{{CVaR}})$ correctly applied zero penalty.
3. **Exp 3 (Numerator Ranking vs Volatility Position Sizing):**
   - Separates the ranking effect of dividing by volatility from entry sizing. Compares equal sizing under numerator ranking vs dynamic inverse-volatility capital allocation.
4. **Exp 5 (Mean-Score Ensemble):**
   - Directly executes $\\overline{{S}} = \\frac{{1}}{{3}}\\sum_s S_s$ as a single unified portfolio. Correlation between multi-seed disagreement $\\sigma_s$ and absolute 63-day prediction error: **$r = {disagree_corr:+.4f}$**.
"""
