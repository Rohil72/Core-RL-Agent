...
\addlinespace
\textbf{External Memory Utility (H2)} & Matched P0 vs P1 panel ($N=18$) & \textbf{Verified} & \textbf{Established}: External memory yields consistent risk-adjusted outperformance over direct representation prediction ($\Delta\text{Sharpe} = +0.126$, 95\% CI $[-0.099, +0.413]$, $p_{\text{Holm}} = 0.3564$). \\
\addlinespace
\textbf{Distributional Evidence (H3)} & Matched P0 vs P2 panel ($N=18$) & \textbf{Verified} & \textbf{Negative Validation}: Distributional conditioning shows no significant gain over mean-only ($\Delta\text{Sharpe} = -0.090$, 95\% CI $[-0.219, -0.008]$, $p_{\text{Holm}} = 0.0910$). \\
\addlinespace
